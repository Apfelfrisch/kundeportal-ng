...
final class EdiLoadProfilesTest extends ChartsTestCase
{
    public function test_the_endpoint_is_hidden_when_the_feature_flag_is_off(): void
    {
        // The dynamic-prices flag alone must not expose the EDI chart.
    ...
    public function test_guests_are_rejected(): void
    {
        ...
        $this->getJson("/api/customers/{$user->id}/contracts/123456/edi-load-profiles")
            ->assertUnauthorized();
    }
    ...
    public function test_a_foreign_customer_is_rejected(): void
    {
        ...
        $this->actingAs($user)
            ->getJson("/api/customers/{$other->id}/contracts/123456/edi-load-profiles")
            ->assertForbidden();
    }
    ...
    public function test_the_requested_day_returns_the_usage_entries(): void
    {
        $this->enableFeature('edi-load-profiles');
        ...
        $this->actingAs($user)
            ->getJson("/api/customers/{$user->id}/contracts/123456/edi-load-profiles?date=2025-06-10")
            ->assertOk()
            ...
            ->assertJsonPath('data.entries.0.usage_kwh', 0.42)
            ->assertJsonPath('data.entries.1.from', '2025-06-10T00:15:00')
            ->assertJsonPath('data.entries.1.usage_kwh', 0.31)
    ...
    public function test_the_center_defaults_to_the_last_reading_date(): void
    {
        $this->enableFeature('edi-load-profiles');
    ...
    public function test_the_center_falls_back_to_yesterday_when_no_reading_date_exists(): void
    {
        $this->enableFeature('edi-load-profiles');
        ...
        MockClient::global([
            GetContractRequest::class => new MockResponse(['data' => $this->contractPayload()]),
            GetLastReadingDateRequest::class => new MockResponse(['last_reading_date' => null]),
            GetEdiLoadProfilesRequest::class => new MockResponse(FixtureLoader::raw('customer-data-api/edi-load-profiles.json')),
        ]);
    ...
    public function test_the_center_falls_back_to_yesterday_when_the_reading_date_call_fails(): void
    {
        $this->enableFeature('edi-load-profiles');
        ...
        MockClient::global([
            GetContractRequest::class => new MockResponse(['data' => $this->contractPayload()]),
            GetLastReadingDateRequest::class => new MockResponse([], 500),
            GetEdiLoadProfilesRequest::class => new MockResponse(FixtureLoader::raw('customer-data-api/edi-load-profiles.json')),
        ]);
    ...
    public function test_a_window_without_data_yields_an_empty_entry_list(): void
    {
        ...
        MockClient::global([
            GetContractRequest::class => new MockResponse(['data' => $this->contractPayload()]),
            GetEdiLoadProfilesRequest::class => new MockResponse(['data' => []]),
        ]);
    ...
    public function test_an_administrator_bypasses_the_assignment_check(): void
    {
        ...
        $this->actingAs($admin)
            ->getJson("/api/customers/{$customer->id}/contracts/123456/edi-load-profiles?date=2025-06-10")
            ->assertOk()
            ->assertJsonCount(2, 'data.entries');
    }
}
