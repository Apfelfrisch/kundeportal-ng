...
/** Farbfeld je Eintrag: ein Kostenanteil, die Preislinie oder der blasse (vorläufige) Balken. */
const SWATCHES = {
...
const ENTRIES: ReadonlyArray<{ swatch: keyof typeof SWATCHES; title: string; text: string }> = [
    {
      swatch: 'exchange',
      title: 'Börsenpreis',
...
/** Aufklappbare Erklärung der Farben, der Preislinie und der blassen Balken. */
      ...
      {open ? (
        <View style={[styles.body, { borderTopColor: theme.divider }]}>
          <Txt variant="muted" style={styles.paragraph}>
            In kWh zeigt jeder Balken deinen Verbrauch. In € zeigt er die Kosten des Zeitraums, gestapelt nach den drei Preisbestandteilen. Alle Preise netto.
          </Txt>
          {ENTRIES.map((entry) => (
